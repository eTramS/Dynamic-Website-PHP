<section id="team" class="row g-0 py-0 text-center">
      <div class="col  team-member">
         <div class="team-member-img">
            <img src="img/jerome.jpg" alt="">
            <div class="social-icons">
               <a href="https://www.facebook.com/jerome.ariola.334"><i class="bx bxl-facebook"></i></a>
               <a href="#"><i class="bx bxl-twitter"></i></a>
               <a href="#"><i class="bx bxl-instagram"></i></a>
               <a href="#"><i class="bx bxl-github"></i></a>
            </div>
         </div>
         <div class="p-4">
            <h5 class="title-sm mt-3 mb-0 text-white">Jerome Ariola</h5>
            <small class="text-white">2D Artist</small>
            <div class="hline"></div>
            <p class="text-white">Well skillful lorem Ipsum is not simply random text. It has roots in a piece of classical Latin literature from 45 BC</p>
         </div>
      </div>
      <div class="col  team-member even">
         <div class="team-member-img">
            <img src="img/lander2.png" alt="">
            <div class="social-icons">
               <a href="https://www.facebook.com/Itb.Dhen"><i class="bx bxl-facebook"></i></a>
               <a href="#"><i class="bx bxl-twitter"></i></a>
               <a href="#"><i class="bx bxl-instagram"></i></a>
               <a href="#"><i class="bx bxl-github"></i></a>
            </div>
         </div>
         <div class="p-4">
            <h5 class="title-sm mt-3 mb-0 text-white">Lander Biag</h5>
            <small class="text-white">3D Artist</small>
            <div class="hline"></div>
            <p class="text-white">Well skillful lorem Ipsum is not simply random text. It has roots in a piece of classical Latin literature from 45 BC</p>
         </div>
      </div>
      <div class="col  team-member">
         <div class="team-member-img">
            <img src="img/steven.jpg" alt="">
            <div class="social-icons">
               <a href="https://www.facebook.com/Charis.Jinxuu"><i class="bx bxl-facebook"></i></a>
               <a href="#"><i class="bx bxl-twitter"></i></a>
               <a href="#"><i class="bx bxl-instagram"></i></a>
               <a href="#"><i class="bx bxl-github"></i></a>
            </div>
         </div>
         <div class="p-4">
            <h5 class="title-sm mt-3 mb-0 text-white">Steven De Juan</h5>
            <small class="text-white">Unity Developer</small>
            <div class="hline"></div>
            <p class="text-white">Well skillful lorem Ipsum is not simply random text. It has roots in a piece of classical Latin literature from 45 BC</p>
         </div>
      </div>
      <div class="col  team-member even">
         <div class="team-member-img">
            <img src="img/jerald.jpg" alt="">
            <div class="social-icons">
               <a href="https://www.facebook.com/niyo.juan.22/"><i class="bx bxl-facebook"></i></a>
               <a href="#"><i class="bx bxl-twitter"></i></a>
               <a href="#"><i class="bx bxl-instagram"></i></a>
               <a href="#"><i class="bx bxl-github"></i></a>
            </div>
         </div>
         <div class="p-4">
            <h5 class="title-sm mt-3 mb-0 text-white">Mark Jerald Infante</h5>
            <small class="text-white">Unity Developer</small>
            <div class="hline"></div>
            <p class="text-white">Well skillful lorem Ipsum is not simply random text. It has roots in a piece of classical Latin literature from 45 BC</p>
         </div>
      </div>
      <div class="col team-member">
         <div class="team-member-img">
            <img src="img/vincent.jpg" alt="">
            <div class="social-icons">
               <a href="https://www.facebook.com/vincent.sanbuenaventura"><i class="bx bxl-facebook"></i></a>
               <a href="#"><i class="bx bxl-twitter"></i></a>
               <a href="#"><i class="bx bxl-instagram"></i></a>
               <a href="#"><i class="bx bxl-github"></i></a>
            </div>
         </div>
         <div class="p-4">
            <h5 class="title-sm mt-3 mb-0 text-white">Vincent San Buenaventura</h5>
            <small class="text-white">3D Artist</small>
            <div class="hline"></div>
            <p class="text-white">Well skillful lorem Ipsum is not simply random text. It has roots in a piece of classical Latin literature from 45 BC</p>
         </div>
   
   </section>